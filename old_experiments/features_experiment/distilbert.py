...
"""
DistilBERT Regression Model for Question Difficulty Prediction

This script implements a regression model based on the DistilBERT base uncased model
to predict question difficulty. It uses the question text and options as input
and predicts a continuous difficulty score. DistilBERT is a lighter and faster version
of BERT with similar performance.
"""
...
def tokenize_function(examples):
    """
    Tokenize text for DistilBERT model (obsolete, kept for reference)
...
def plot_predictions(y_true, y_pred, file_path="prediction_scatter_distilbert.png"):
    """
    ...
    plt.title('DistilBERT Model: Actual vs Predicted Difficulty')
    plt.savefig(file_path)
    plt.close()
    logger.info(f"Prediction scatter plot saved to {file_path}")

def save_metrics(metrics, file_path="distilbert_model_metrics.txt"):
    """
...
def train_distilbert_model(
    ...
    epochs=5,
    batch_size=16,  # Larger batch size since DistilBERT is more memory efficient
    learning_rate=5e-5,  # Slightly higher learning rate often works well with DistilBERT
    test_size=0.2,
    random_state=42,
    push_to_hub=False,
    hub_repo_id=None,
    ...
    """
    Train a DistilBERT model for question difficulty prediction
    ...
    # Load DistilBERT model for regression (sequence classification with 1 label)
...
def predict_difficulty(question_text, options, model_path="distilbert_model"):
    """
...
def update_metrics_comparison(metrics, difficulty_threshold=None):
    """
    Update the metrics comparison file with DistilBERT model results
    ...
    try:
        ...
        # Create entry for DistilBERT model
        if difficulty_threshold is not None:
            model_name = f"DistilBERT base uncased (Filtered: difficulty >= {difficulty_threshold})"
        ...
        logger.info(f"Updated metrics comparison file with DistilBERT model results.")
...
def main():
    """
    Main function to run the DistilBERT model training
    """
    ...
    # Train the model
    metrics = train_distilbert_model(
        data_file=data_file,
        output_dir=output_dir,
        difficulty_threshold=difficulty_threshold,
        epochs=5,
        batch_size=16,  # Larger batch size than BERT since DistilBERT is more memory efficient
        learning_rate=5e-5  # Slightly higher learning rate often works well with DistilBERT
    )
