...
"""
LightGBM Model for Question Difficulty Prediction (With MPNet Embeddings)
...
def preprocess_features(df):
    ...
    with open('feature_processors_lightgbm_mpnet_embeddings.pkl', 'wb') as f:
...
def train_and_evaluate_model(X, y, feature_names):
    ...
    # Save feature importance to CSV
    feature_importance_file = "feature_importance_mpnet_embeddings.csv"
    feature_importance_df.to_csv(feature_importance_file, index=False)
    ...
    plt.xlabel('Actual Difficulty')
    plt.ylabel('Predicted Difficulty')
    plt.title('LightGBM with MPNet Embeddings: Actual vs Predicted Difficulty')
    plt.savefig('lightgbm_mpnet_embeddings_predictions.png')
    plt.close()
    
    # Update metrics comparison
    update_metrics_comparison("LightGBM_MPNet_Embeddings", test_rmse, test_mae, test_r2)
