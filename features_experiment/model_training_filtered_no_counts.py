...
"""
LightGBM Model for Question Difficulty Prediction (Without Response Counts)
...
def preprocess_features(df, use_id_features=True):
    ...
    with open('feature_processors_filtered_no_counts.pkl', 'wb') as f:
...
def train_and_evaluate(X, y, feature_names, use_tuning=False):
    ...
    # Save model
    with open('question_model_filtered_no_counts.pkl', 'wb') as f:
        pickle.dump(model, f)
    ...
    plt.title('Actual vs Predicted Difficulty (Filtered Model, No Count Features)')
    plt.savefig('prediction_scatter_filtered_no_counts.png')
...
def analyze_feature_importance(model, feature_names):
    ...
    feature_importance.to_csv('feature_importance_filtered_no_counts.csv', index=False)
    ...
    plt.ylabel('Feature')
    plt.title('Top 20 Features (Filtered Model, No Count Features)')
    plt.tight_layout()
    plt.savefig('feature_importance_filtered_no_counts.png')

def update_metrics_comparison(cv_scores, test_rmse, test_mae, test_r2):
    """
    Update the metrics comparison file with results from the filtered model without count features.
    ...
    try:
        ...
        # Create new entry for filtered model without count features
        new_entry = (
        ...
        if lgbm_section >= 0 and next_section >= 0:
            ...
            print("Updated metrics_comparison.txt with filtered model (no count features) results.")
        else:
...
def main():
    """Main function to run the filtered model training pipeline without count features."""
    ...
    if 'irt_difficulty' in df.columns:
        ...
        print("- question_model_filtered_no_counts.pkl: The trained model")
        print("- feature_processors_filtered_no_counts.pkl: Feature processing objects")
        print("- feature_importance_filtered_no_counts.csv: Feature importance data")
        print("- feature_importance_filtered_no_counts.png: Visualization of feature importance")
        print("- prediction_scatter_filtered_no_counts.png: Scatter plot of actual vs predicted values")
